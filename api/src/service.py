...
from api.src.constants import FormulaOneUrls, AAI_CONFIG
...
def driver_race_radio_data(drivers_df: pd.DataFrame,
    ...
    return {"lap_duration": session_lap_time_mode,
            "lap_sentiment": lap_sent_avg.to_dict(orient="records"),
...
def _transcribe_radio_recordings(radios_df: pd.DataFrame):
    ...
    for rec_url in recordings:
        ...
        if transcript.error:
            transcriptions[-1]["sentiment"] = ""
            transcriptions[-1]["conversation_analysis"] = []
            continue
        ...
        transcriptions[-1]["sentiment"] = (pd.DataFrame(radio_sentiment)["sentiment"].map({"NEGATIVE": -1,
                                                                                            ...
                                                                                            "POSITIVE": 1})
                                                                                            .mean())
        transcriptions[-1]["conversation_analysis"] = transcript.sentiment_analysis
...
def _avg_sentiment_by_lap(radios_df: pd.DataFrame):
    """ Returns dataframe with lap by lap average sentiment. """
    return radios_df[["lap_number", "sentiment"]].groupby(["lap_number"]).mean()
